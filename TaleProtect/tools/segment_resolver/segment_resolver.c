...
i32 phdr_callback(struct dl_phdr_info *info, sizet size, void *arg) {
    ...
    u8t *lowest_start = NULL;   // will track lowest PT_LOAD start
    u8t *exec_end = NULL;       // end of the exec segment
    ...
    for (int i = 0; i < info->dlpi_phnum; i++) {
        ...
        // track lowest LOAD segment start for full hashing range
        if (p->p_type == PT_LOAD && (!lowest_start || seg_start < lowest_start)) {
            lowest_start = seg_start;
        }

        LOGI("(CB) PHDR[%d]: type=%d, flags=0x%x, start=%p, size=%zu",
             i, p->p_type, p->p_flags, (void *) seg_start, seg_size);

        /* [exec segment] */
        if (!d->seg->exec.start && (p->p_flags & PF_X) && p->p_type == PT_LOAD) {
            d->seg->exec.start = seg_start;
            d->seg->exec.size = seg_size;
            exec_end = seg_start + seg_size;  // end of exec
            LOGI("(CB) -> chosen as exec segment (start=%p, size=%zu, end=%p)",
                 d->seg->exec.start, d->seg->exec.size, exec_end);
        }
        ...
        if ((p->p_flags & PF_R) && !(p->p_flags & PF_W) && !(p->p_flags & PF_X)) {
            if (d->seg->ro_count < MAX_RO_SEGMENTS) {
                ...
                LOGI("(CB) -> added RO segment[%d]: start=%p, size=%zu",
                     d->seg->ro_count, seg_start, seg_size);
                d->seg->ro_count++;
            }
        }
    }

    // set final base and end_of_exec
    if (lowest_start) {
        d->seg->base = lowest_start;  // start from lowest LOAD segment
    }
    if (exec_end) {
        d->seg->end_of_exec = exec_end; // up to end of exec segment
    }
