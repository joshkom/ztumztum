#include "wrap_sysprop_protection.h"
...
//! wrap.%s
static inline __attribute__((always_inline)) unsigned char *decode_wrap_fmt__7(void) {
    size_t __hc_len = 7u;
    ...
    __hc_tmpB = 0xC94B9C185C9DC01CULL;
...
static inline __attribute__((always_inline)) unsigned char *decode_wrap_com_tale_royale__20(void) {
    ...
    __hc_tmpA = (((((__hc_tmpA * 0x000000009D5E4B49ULL) >> 38) |
                   ((__hc_tmpA * 0x000000009D5E4B49ULL) << 26)) & 0xFFFFFFFFFFFFFFFFULL) ^
                 0x0000000036970A05ULL);
    __hc_tmpB = 0xD8CB9C185C9DDB5BULL;
    __hc_tmpB = ((((((__hc_tmpB >> 50) | (__hc_tmpB << 14)) & 0xFFFFFFFFFFFFFFFFULL) << 36) |
                  ((((__hc_tmpB >> 50) | (__hc_tmpB << 14)) & 0xFFFFFFFFFFFFFFFFULL) >> 28)) &
                 0xFFFFFFFFFFFFFFFFULL);
    buf[0] = (unsigned char) ((__hc_tmpB >> 0) & 0xFFULL);
    ...
    __hc_tmpB = 0xE349EA20DBF5E2E0ULL;
    ...
    __hc_tmpB = 0x5E40000063BBAED6ULL;
...
injdetection_wrap_sysprop_inspection_t injdetection_wrap_sysprop_inspection(void) {
    injdetection_wrap_sysprop_inspection_t r = {0};
    i8 value[PROP_VALUE_MAX];
    i32 len;

    if (!TaleProtectContext.package_name[0]) {
        LOGW("(ProcessMonitor::InjectionDetection::WrapSyspropProtection::Inspection) empty pkg name");
        r.result = 2;
        return r;
    }

    const i8 *decoded_wrap_com_tale_royale = (i8 *) decode_wrap_com_tale_royale__20();

    unstdmemory_compat_memset(value, 0, sizeof(value));
    len = __system_property_getM(decoded_wrap_com_tale_royale, value);
    if (len > 0) {
        LOGE("(ProcessMonitor::InjectionDetection::WrapSyspropProtection::Inspection) wrap property '%s' present, value='%s' (len=%d)",
             decoded_wrap_com_tale_royale, value, len);
        r.result = 1;
        unstdmemory_compat_memcpy(r.wrap_value, value, len);
    } else {
        LOGD("(ProcessMonitor::InjectionDetection::WrapSyspropProtection::Inspection) wrap property '%s' not set",
             decoded_wrap_com_tale_royale);
        r.result = 0;
    }

    secure_memory_wipe(
            (void *) decoded_wrap_com_tale_royale,
            20,
...
void injdetection_wrap_sysprop_mitigation(injdetection_wrap_sysprop_inspection_t buffer_arg) {
    if (buffer_arg.result != 0 || buffer_arg.wrap_value[0]) {
        LOGE("(ProcessMonitor::InjectionDetection::WrapSyspropProtection::Mitigation) 0xbadd00ddeadface...");
        demolisher_stackoverflower();
        demolisher_smash();
    }
}

void injdetection_wrap_sysprop_protection(void) {
    injdetection_wrap_sysprop_mitigation(injdetection_wrap_sysprop_inspection());
}
