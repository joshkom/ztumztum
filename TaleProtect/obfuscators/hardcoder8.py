...
import argparse, hashlib, random, textwrap
...
def make_func_name(data: bytes) -> str:
    prefix = make_hex_prefix(data, max_bytes=6)
    return f"decode_{prefix}__{len(data)}"

def make_macro_name(data: bytes) -> str:
    prefix = make_hex_prefix(data, max_bytes=6)
    return f"DECODE_{prefix}__{len(data)}".upper()
...
def build_c(input_bytes: bytes, seed: str, aggressive: bool, mode: str):
    ...
    # out.append("static inline unsigned long long rol64(unsigned long long x,int n){return ((x<<n)|(x>>(64-n)))&0xFFFFFFFFFFFFFFFFULL;}")
    # out.append("static inline unsigned long long ror64(unsigned long long x,int n){return ((x>>n)|(x<<(64-n)))&0xFFFFFFFFFFFFFFFFULL;}")

    # Function mode (unchanged)
    if mode == 'func':
        ...
        out.append("}")
        out.append(f"static inline __attribute__((always_inline)) static inline size_t {func_name}_len(void){{ return (size_t){n}u; }}")

    # Macro mode: now supports arbitrary length by emitting a compound expression that allocates & returns buf
    elif mode == 'macro':
        # Unique local names to reduce collisions: __hc_buf_<prefix>
        prefix = make_hex_prefix(input_bytes, max_bytes=6)
        buf_name = f"__hc_buf_{prefix}"
        len_name = f"__hc_len_{prefix}"
        tmpA_name = "__hc_tmpA"
        tmpB_name = "__hc_tmpB"

        out.append(f"/* Macro mode: allocates and returns a malloc'ed buffer. Uses GCC/Clang compound-statement extension. */")
        # out.append(f"#define {macro_name} ({ {{} }})")  # placeholder; we'll replace properly below

        # Remove placeholder entirely
        # out.append(f"#define {macro_name} ({ {{} }})")  # <-- remove this

        # Generate macro as single compound-expression
        # macro_single = "({ " + " ".join(l.rstrip(" \\") for l in body) + " })"
        # out.append(f"#undef {macro_name}")
        # out.append(f"#define {macro_name} {macro_single}")
        # out.append(f"#define {macro_name}_LEN ({n}u)")

        out.append(f"#define {macro_name} ({{}})")  # literal braces in output


        # Build the compound-expression body
        body = []
        ...
        for b in range(num_blocks):
            ...
            # when seq_to_c_expr includes __hc_tmpA it matches tmpA_name above (we used same name)
        ...
        # Construct the macro text using the compound expression syntax: ({ <body> })
        # We need to escape newlines properly for C macro
        macro_lines = ["#undef " + macro_name]  # ensure clean
        macro_lines.append(f"#define {macro_name} ({'{'}")  # start compound; use '{' then '}' substituted later
        # Append each body line with a trailing backslash for macro continuation
        for line in body:
            macro_lines.append(f"    {line} \\")
        # last line should not have trailing backslash; replace last trailing backslash by closing expression
        # But easier: assemble as single-line macro with semicolons and final value, wrapped in ({ ... })
        macro_single = "({ " + " ".join(l.rstrip(" \\") for l in body) + " })"
        # Replace the earlier placeholder with the actual macro definition
        # Remove the placeholder line and append the single-line macro
        out.pop()  # remove placeholder
        out.append(f"#undef {macro_name}")
        out.append(f"#define {macro_name} {macro_single}")
        out.append(f"#define {macro_name}_LEN ({n}u)")
