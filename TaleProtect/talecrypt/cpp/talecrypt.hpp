...
#define ERR_AND_SMSH(args) demolisher_smash()
...
//        static std::vector<uint8_t> hex_to_bytes(const char *hex);
        ...
        static T deserialize_struct(const std::vector<uint8_t> &buffer) {
            if (buffer.size() != sizeof(T)) {
                ERR_AND_SMSH("Invalid buffer size");
            }
            T obj;
        ...
        // Generate a random 256-bit key (static helper)
        ...
        static std::string static_decrypt_string_from_hex(const std::string &hex_ciphertext,
                                                          const std::vector<uint8_t> &key,
                                                          const std::vector<uint8_t> &iv);
        ...
        void serialize_and_encrypt_to_file(const T &data,
            ...
            if (!out.is_open()) {
                ERR_AND_SMSH("Failed to open file for writing.");
            }
        ...
        // Optional: getter for current key
