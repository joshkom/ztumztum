...
bool talecrypt::AES256Encryptor::encrypt_file(const std::string &input_path,
    ...
    if (!in.read(reinterpret_cast<char *>(buffer.data()), size)) {
        ERR_AND_SMSH("Failed to read input file");
    }
...
bool talecrypt::AES256Encryptor::decrypt_file(const std::string &input_path,
    ...
    std::ifstream in(input_path, std::ios::binary | std::ios::ate);
    if (!in.is_open()) {
        ERR_AND_SMSH("Failed to open encrypted file");
    ...
    if (!out.write(reinterpret_cast<const char *>(decrypted.data()), decrypted.size())) {
        ERR_AND_SMSH("Failed to write decrypted data");
    }
...
//    if (len % 2 != 0) {
//        ERR_AND_SMSH("Invalid hex length");
//        return {};
//    }
...
//        if ('A' <= c && c <= 'F') return c - 'A' + 10;
//        ERR_AND_SMSH("Invalid hex character");
//        return 0;
//    };
...
talecrypt::AES256Encryptor::encrypt(const std::vector<uint8_t> &plaintext,
    ...
    if (iv.size() != 16) ERR_AND_SMSH("IV must be exactly 16 bytes");

    EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
    if (!ctx) ERR_AND_SMSH("Failed to create cipher context");

    if (1 != EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, key_.data(), iv.data()))
        ERR_AND_SMSH("EncryptInit failed");
    ...
    if (1 !=
        ...
        ERR_AND_SMSH("EncryptUpdate failed");
    ciphertext_len = len;

    if (1 != EVP_EncryptFinal_ex(ctx, ciphertext.data() + len, &len))
        ERR_AND_SMSH("EncryptFinal failed");
    ciphertext_len += len;
...
talecrypt::AES256Encryptor::decrypt(const std::vector<uint8_t> &ciphertext,
    ...
    if (iv.size() != 16) ERR_AND_SMSH("IV must be 16 bytes");
...
std::vector<uint8_t> talecrypt::AES256Encryptor::decrypt_from_file(const std::string &filepath,
    ...
    if (!in.is_open()) ERR_AND_SMSH("Failed to open file for decryption");

    std::streamsize size = in.tellg();
    if (size <= 0) ERR_AND_SMSH("Invalid or empty encrypted file");

    in.seekg(0, std::ios::beg);
    std::vector<uint8_t> encrypted(size);
    if (!in.read(reinterpret_cast<char *>(encrypted.data()), size)) {
        ERR_AND_SMSH("Failed to read encrypted data");
    }
...
std::string
talecrypt::AES256Encryptor::static_decrypt_string_from_hex(const std::string &hex_ciphertext,
                                                           const std::vector<uint8_t> &key,
                                                           const std::vector<uint8_t> &iv) {
    AES256Encryptor decryptor(key);
    return decryptor.decrypt_string_from_hex(hex_ciphertext, iv);
}
